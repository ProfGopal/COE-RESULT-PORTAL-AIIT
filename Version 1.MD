# MASTER ARCHITECTURE BLUEPRINT: AIIT Version 9.0 (Complete UI, Filter & Curriculum Editor Suite)

You are updating a static GitHub Pages site (`script.js`, `admin-hidden.html`, `index.html`). We are restoring student portal tabs (Degree Audit & Active Backlogs), fixing faculty directory filters and "View All Students" loading, removing whitespace layout gaps, enabling global PDF/Excel exports across all individual and collective views, and rebuilding the Admin Curriculum Manager into an editable table format with category renaming, credit adjustments, and course management, updating version tags to **Ver 9.0**, and creating `VERSION9.0.md`.

Please execute the following strict updates:

## 1. Global Versioning Update
* **File:** `index.html`, `admin-hidden.html`
  * Change all version tags in the top left corners to `Ver 9.0`.

## 2. Master Unified Engine (`script.js`)
* **File:** `script.js`
  * Replace your entire `script.js` file with this complete, highly-optimized cloud engine:
...
## 3. Create `VERSION9.0.md`
* Create a file named `VERSION9.0.md` in your project root containing the release notes for Ver 9.0.